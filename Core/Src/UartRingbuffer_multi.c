...
again:
	...
	if(uart == wifi_uart)
	{
		while (Uart_peek() != string[so_far])  // peek in the rx_buffer to see if we get the string
		{
			...
			{
				_rx_buffer1->tail = (unsigned int)(_rx_buffer1->tail + 1) % UART_BUFFER_SIZE;  // increment the tail
			}
	...
	if(uart == pc_uart)
	{
		while (Uart_peek() != string[so_far])  // peek in the rx_buffer to see if we get the string
		{
			...
			{
				_rx_buffer2->tail = (unsigned int)(_rx_buffer2->tail + 1) % UART_BUFFER_SIZE;  // increment the tail
			}
